...
<style>
body{display:flex;flex-direction:column;justify-content:center;min-height:0vh;color:#000;background-image:url(img/background.jpg);background-size:cover;-webkit-background-size:cover;background-repeat:no-repeat}.btn{-moz-appearance:none;cursor:pointer;margin:5px;align-items:center;border:2px solid #132130;border-radius:3px;background:#132130}.btn:hover,.btn:focus{color:#000;outline:0}.geser{border-color:#132130;color:#fff;padding:3px 20px;background-image:linear-gradient(45deg,#00ACD0 50%,transparent 50%);background-position:100%;background-size:400%;transition:background 300ms ease-in-out}.geser:hover{background-position:0}.script_txt{text-align:left;font-family:cursive;font-weight:700;color:#000;border:8px;border-radius:10px;background:hsla(0,0%,100%,.3);align-items:center;width:395px;height:50px}.col-butt{text-align:center;border:5px;align-items:center}.inline-block{display:inline-block;text-align:left;margin:5px;top:0}.box_script{width:450px;border:none;border-radius:10px;margin:3% auto;padding:10px 10px;background-color:#000;box-shadow:0 0 5px 2px #132130}.footer{width:100%;height:50px;display:flex;justify-content:center;align-items:center;color:#fff;font-weight:700;bottom:0;z-index:1;background:repeating-linear-gradient(-45deg,red 0%,yellow 7.14%,rgb(0,255,0) 14.28%,rgb(0,255,255) 21.4%,cyan 28.56%,blue 35.7%,magenta 42.84%,red 50%);background-size:600vw 600vw;-webkit-text-fill-color:transparent;-webkit-background-clip:text;animation:slide 10s linear infinite forwards}@keyframes slide{0%{background-position-x:0%}100%{background-position-x:600vw}}.blink{-webkit-animation:blink .75s linear infinite;-moz-animation:blink .75s linear infinite;-ms-animation:blink .75s linear infinite;-o-animation:blink .75s linear infinite;animation:blink .75s linear infinite;color:#00ACD0;font-weight:700}@-webkit-keyframes blink{0%{opacity:1}50%{opacity:1}50.01%{opacity:0}100%{opacity:0}}@-moz-keyframes blink{0%{opacity:1}50%{opacity:1}50.01%{opacity:0}100%{opacity:0}}@-ms-keyframes blink{0%{opacity:1}5O%{opacity:1}50.01%{opacity:0}100%{opacity:0}}@-o-keyframes blink 0%{opacity:1}50%{opacity:1}50.01%{opacity:0}100%{opacity:0}}@keyframes blink{0%{opacity:1}50% opacity:1}50.01%{opacity:0}100%{opacity:0}}		
</style>
<script>
function shipping_calc(){var val=document.getElementById("idconf").value;if(val==="config1"){var data=document.getElementById("isi1").value;document.getElementById("isi").value=data}
if(val==="config2"){var data=document.getElementById("isi2").value;document.getElementById("isi").value=data}
if(val==="config3"){var data=document.getElementById("isi3").value;document.getElementById("isi").value=data}
if(val==="config4"){var data=document.getElementById("isi4").value;document.getElementById("isi").value=data}
if(val==="config5"){var data=document.getElementById("isi5").value;document.getElementById("isi").value=data}}
</script>
<script type="text/javascript">
$(document).ready(function(){setInterval(function(){$.ajax({url:"screenlog.0",cache:!1,success:function(result){$("#log").html(result)}});$(document).ready(function(){$.ajaxSetup({cache:!1})});var textarea=document.getElementById("log");textarea.scrollTop=textarea.scrollHeight},1000)});if(window.history.replaceState){window.history.replaceState(null,null,window.location.href)}
</script>
...
function getorg(json) {
    document.write("/ ", json.org);
  }

</script>

<script type="application/javascript" src="https://ipinfo.io/?format=jsonp&callback=getIP"></script>
<script type="application/javascript" src="https://ipinfo.io/?format=jsonp&callback=getcity"></script>
<script type="application/javascript" src="https://ipinfo.io/?format=jsonp&callback=getorg"></script>
</div>
...
if ( $dns[0] == "yes"){
echo '<input type="checkbox" name="" value="yes" checked>DNS-Resolver'; }
else {
exec("echo 'use_dns=no' > config/dns");
echo '<input type="checkbox" name="" value=".">DNS-Resolv'; }
